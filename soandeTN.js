...
async function loadAllMetadata(isRefresh = false) {
  ...
  try {
    ...
    if (headerEl) headerEl.textContent = 'Sẵn sàng. Nhấn nút Lọc để xem câu hỏi';
...
async function editMaDe(maDeId) {
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  try {
    ...
    const btnFilter = document.getElementById('btnFilter'); if (btnFilter) { btnFilter.innerHTML = '<i class="fas fa-filter mr-2"></i>'; btnFilter.addEventListener('click', applyFilter); }
