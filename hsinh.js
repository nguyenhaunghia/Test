  ...
  try {
      ...
      tasks.forEach(t => {
        ...
        if (isNotStarted) {
            ...
            initBtnText = isTL ? 'Chưa hoàn thành nhiệm vụ học tập, nộp bài ngay' : 'Chưa hoàn thành nhiệm vụ học tập, làm bài ngay';
        ...
        if (isTL) {
            middleContent = `
              <div class="flex-1 bg-indigo-50 border border-indigo-100 rounded-xl p-4 relative overflow-hidden group">
                <div class="absolute top-0 right-0 p-2 opacity-5"><i class="fas fa-quote-right text-4xl text-indigo-900"></i></div>
                <h4 class="text-xs font-bold text-indigo-600 uppercase mb-2 flex items-center"><i class="fas fa-exclamation-circle mr-2"></i> Yêu cầu làm bài</h4>
                <div class="text-sm text-slate-700 leading-relaxed max-h-[100px] overflow-y-auto custom-scroll pr-2 whitespace-pre-wrap">${note}</div>
              </div>`;
        } else {
            middleContent = `
        ...
        card.className = 'card-glass p-5 md:p-6 rounded-2xl relative overflow-hidden flex flex-col h-full';
        
        card.innerHTML = `
          <div class="w-full mb-4 border-b border-slate-200/60 pb-3">
             <h3 class="text-xl md:text-2xl font-bold text-indigo-950 uppercase tracking-wide leading-tight">${topic}</h3>
          </div>
          
          <div class="flex flex-col lg:flex-row gap-6 flex-1 h-full">
            ...
            <div class="lg:w-[45%] border-t lg:border-t-0 lg:border-l border-slate-200 pt-5 lg:pt-0 lg:pl-6 flex flex-col h-full min-h-[160px]">
                ...
                <div class="flex-1 bg-white/70 rounded-xl relative overflow-hidden border border-slate-200 shadow-inner">
        ...
        callAPI('getLichSuLamBai', { TestID: testID, UserID: currentUser.UserID, SubjectID: subjectID }, true).then(histRes => {
           ...
           if(histData.length === 0) {
             ...
             histData.slice().reverse().forEach(r => {
               ...
               // Ép khoảng cách các dòng nhỏ lại (py-1.5 px-2)
               if (isTL) {
             ...
             if (btn && !isNotStarted) {
                 ...
                 if (isExpired) {
                     ...
                     if (isAchieved) {
                         ...
                         btn.innerHTML = '<i class="fas fa-crown text-yellow-300"></i> Chúc mừng bạn đã hoàn thành tốt nhiệm vụ học tập';
                         btn.onclick = null;
                         btn.disabled = true;
                     }
                 }
